...
class SimpleAIDecidingModule(DecidingModule):   
    ...
    def decide(self, databit: Candles, trend: list | None = None, predictions: list[ndarray] | None = None, risks: list | None = None) -> bool | None:
        if any(risks): return False
        if self._allow_trading(trend):
            avg_pred = np.mean([p[0][1] for p in predictions])
