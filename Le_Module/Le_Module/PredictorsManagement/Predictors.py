...
class AIPredictor(Predictor):
    ...
    def train(self, dataset: tuple, epochs: int=100, verbose: bool=False, callbacks:list | None=None, v_split: float=0.1) -> None:
        self.model.fit(x=dataset[0], y=dataset[1], epochs=epochs, shuffle=True, verbose=verbose, callbacks=callbacks, validation_split=v_split, batch_size=32)
   
    def biased_predict(self, databit: np.ndarray) -> np.ndarray:
        try:
            databit = np.reshape(databit, [1]+list(self.model.input_shape[1:]))
        except:
            print(f'seems like the sample (with shape: {databit.shape}) cannot be reshaped to the model`s input_shape: {self.model.input_shape[1:]}')
        
        p = self.model.predict(databit, verbose=False)
...
class AITrendviewer(AIPredictor):
    ...
    def _examine_performance(self, dataset: Candles):
        ...
        if any([isinstance(x, layers.RNN) for x in self.model.layers]):
            input_len = self.model.input_shape[-1]
        else:
            input_len = self.model.input_shape[-2]
